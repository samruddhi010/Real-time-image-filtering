...
int main(int argc, char *argv[]) {
    ...
    Mat frame = imread("sultan.jpg");
        // cv::imshow("Original Image",image);
        int down_width = 800;
        int down_height = 600;
        Mat resized_down;
        resize(frame, Image, Size(down_width,down_height), cv::INTER_LINEAR);
        // imshow("Resize down by defining height and width", Image);
        if(frame.empty())
        {
            std::cout << "Could not read the image: " << "/home/samruddhi/CV/Project_1" << std::endl;
            return 1;
        }
